...
const getUsers = async (deviceId, from, to, distance, limited) => {
    try {
      ...
      var locationWhere = [];
      location.forEach((l) => {
        var timeFrom = new Date(new Date(l['createdAt']).getTime() - 60000).toISOString();
        var timeTo = new Date(new Date(l['createdAt']).getTime() + 60000).toISOString();
        locationWhere.push({
          [Op.and]: [
            {createdAt: {[Op.between]: [timeFrom, timeTo]}},
            Sequelize.where(Sequelize.fn('ST_DWithin', Sequelize.col('geom'), 'SRID=4326;POINT('+ l['geom']['coordinates'][0] +' '+ l['geom']['coordinates'][1] +')', distance, true), true)
          ]
        });
      });

      if (locationWhere.length === 0) { console.log("Location is empty!"); return [];}
