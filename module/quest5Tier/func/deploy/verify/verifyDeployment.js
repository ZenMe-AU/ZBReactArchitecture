...
(async () => {
  try {
    ...
    if (state !== "Running") {
      throw new Error(
        `Function App ${functionAppName} is not running! Current state: ${state}`,
      );
    }
    ...
    console.log(
      "PDV Step 2: Check if the function app contains expected functions.",
    );
    const list = new Set(getFunctionList(functionAppName, resourceGroupName));
    const missingFunctions = expectedFunctionList.filter(
      (func) => !list.has(func),
    );
...
function getFunctionAppState(functionAppName, resourceGroupName) {
  try {
    ...
    const state = execSync(
      `az functionapp show --name ${functionAppName} --resource-group ${resourceGroupName} --query "properties.state" -o tsv`,
      {
        encoding: "utf8",
      },
    ).trim();
...
function getFunctionList(functionAppName, resourceGroupName) {
  ...
  try {
    return JSON.parse(
      execSync(
        `az functionapp function list --name ${functionAppName} --resource-group ${resourceGroupName} --query "[].config.name" -o json`,
        {
          encoding: "utf8",
        },
      ),
