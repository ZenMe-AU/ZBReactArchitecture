  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4 }}>
                ...
                {resolvedQuestions.map((q: Question, index: number) => (
                  <div key={q.id}>
                    <ListItem disablePadding>
                      <ListItemButton component={Link} to={`/quest3TierCqrs/${q.id}/answer`} onClick={() => handleOpenAnswer(q.id)}>
                        ...
                        <Box display="flex" gap={1} ml={2}>
                          <Button
                            ...
                            startIcon={<EditIcon />}
                            component={Link}
                            to={`/quest3TierCqrs/${q.id}` + (q.profileId !== profileId ? "/add" : "")}
      ...
      <Box mt={4} display="flex" justifyContent="space-between">
        <Button variant="contained" color="primary" component={Link} to="/quest3TierCqrs/add" onClick={() => handleAddQuestion}>
          + Add Question
        </Button>
      </Box>
    </Container>
